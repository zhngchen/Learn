...
class Point < GeometryValue
  ...
  # eval_prog and preprocess_prog are described in GeometryValue
  def shift(dx, dy)
    Point.new(x+dx, y+dy)
  end
  ...
  def intersectPoint pt
    if real_close_point(self.x, self.y, pt.x, pt.y) then self else NoPoints.new end
  end
...
class Line < GeometryValue
  ...
  def intersectPoint pt
    pt.intersectLine(self)
  end

  def intersectLine line
    if real_close(self.m, line.m)
      if real_close(self.b, line.b) then self else NoPoints.new end
    else
...
class VerticalLine < GeometryValue
  ...
  def intersectVerticalLine vline
    if real_close(self.x, vline.x) then self else NoPoints.new end
  end
...
class LineSegment < GeometryValue
  ...
  # eval_prog is described in GeometryValue
  def preprocess_prog
    if real_close_point(x1,y1,x2,y2)
      Point.new(x1, y1)
    elsif x2 < x1 - GeometryExpression::Epsilon or (real_close(x1, x2) and y2 < y1)
...
class Let < GeometryExpression
  ...
  def eval_prog env
    env = env.unshift([@s, @e1.eval_prog(env)])    # add ahead
    @e2.eval_prog(env)
  end
