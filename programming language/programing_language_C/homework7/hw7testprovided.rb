...
# extra Tests
ll = Let.new("y",Let.new("x",NoPoints.new,NoPoints.new),Var.new("x"))



# more intersect
pp = Point.new(5.0,7.0).intersect(LineSegment.new(-1.0,2.0,5.0,7.0))
pp1 = pp.preprocess_prog.eval_prog([])
line_res = Point.new(5.0, 7.0).intersect(Line.new(5.0/6.0, 2 + 5.0/6.0))
# puts line_res.intersectWithSegmentAsLineResult(LineSegment.new(-1.0,2.0,5.0,7.0))


##
ll = Line.new(5.0,0.0).intersect(LineSegment.new(-1.0,-1.0,1.0,5.0))
ll1 = ll.preprocess_prog.eval_prog([])
# puts ("ll1.x: %d, ll1.y: %d" % [ll1.x, ll.y])
