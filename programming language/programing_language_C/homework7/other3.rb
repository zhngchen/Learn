...
#TODO add (fix) intersection methods, add expressions classes
...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    if real_close(@x1, @x2) # segments are on a vertical line
      # a starts at b or below b
      if @y1 < seg.y1 
        a = LineSegment.new(@x1, @y1, @x2, @y2)
      else 
        a = LineSegment.new(seg.x1, seg.y1, seg.x2, seg.y2) 
      end
      if y1 < seg.y1
        b = LineSegment.new(seg.x1, seg.y1, seg.x2, seg.y2)
      else 
        b = LineSegment.new(@x1, @y1, @x2, @y2) 
      end
      ...
      # a starts at b, or to the left of b
      if @x1 < seg.x1 
        a = LineSegment.new(@x1, @y1, @x2, @y2)
      else
        a = LineSegment.new(seg.x1, seg.y1, seg.x2, seg.y2) 
      ...
      if x1 < seg.x1 
        ...
        b = LineSegment.new(@x1, @y1, @x2, @y2) end
