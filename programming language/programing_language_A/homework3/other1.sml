...
list. If the list is empty, return "". In the case of a tie, return the string closest to the beginning of the list. Use foldl, String.size, and no recursion (other than the implementation 
...
(* Problem 3. Write a function longest_string2 that is exactly like longest_string1 except in 
the case of ties it returns the string closest to the end of the list. Your solution should be 
almost an exact copy of longest_string1. Still use foldl and String.size.
*)
...
(* Problem 4. Write functions longest_string_helper, longest_string3, and longest_string4 such that:

- longest_string3 has the same behavior as longest_string1 and longest_string4 has the same behavior 
as longest_string2.
...
fun longest_string_helper compop ss =
    foldl ( fn(x,acc) => if compop ((String.size x), (String.size acc)) then x else acc ) 
          "" 
          ss
...
(* Problem 5. Write a function longest_capitalized that takes a string list and returns the longest string in
...
(* Problem 6. Write a function rev_string that takes a string and returns the string that is the same characters in
...
(* Problem 7. Write a function first_answer of type ('a -> 'b option) -> 'a list -> 'b (notice the 2 argu-
...
Else the calls to the first argument will have produced SOME lst1, SOME lst2, ... SOME lstn and the result of
...
fun all_answers f xs = 
    let 
        ...
        fun helper ( ss ) = 
            case ss of 
                []           => []
              | SOME(s)::ss' => s@helper(ss')
              | NONE   ::ss' => raise None
    in
         SOME(helper(List.map f xs)) handle None => NONE
    end
...
string * valu pairs; order in the list does not matter. The ru
...
(* Problem 9. (This problem uses the pattern datatype but is not really about pattern-matching.) 
...
fun count_some_var (s0 : string, p : pattern) = 
    let 
        fun f1()  = 0
        fun f2(s) = if (s = s0) then 1 else 0
    in
        g f1 f2 p
    end

(* Problem 10. Write a function check_pat that takes a pattern and returns true if and only if all the variables
...
fun check_pat (p : pattern) = 
    let
        fun f1 (p : pattern, acc : string list) =           (* pattern -> string list, returns all strings used as variables  *)
            case p of
                Wildcard          => acc
              | Variable s        => s::acc
              | TupleP ps         => List.foldl (fn (p, acc) => (f1(p, acc))) acc ps
              | ConstructorP(_,p) => f1(p,acc)
              | _                 => acc

        fun f2 (ss : string list) =      (* string list -> bool   , returns true if string list has repeats *)
            case ss of
                []          => false
              | [s1]        => false
              | s1::s2::ss' => (s1=s2) orelse f2(s2::ss')
    in
        not( f2( f1( p,[] ) ) )
    end

(* Problem 11. Write a function match that takes a valu * pattern and returns a (string * valu) list option,
...
Sample solution is 13 lines. Remember to look above for the rules for what patterns match what values, and 
what bindings they produce. 
These are hints: We are not requiring all_answers and ListPair.zip here, but they make it easier.
*)

fun match ( v : valu, p : pattern ) =             (* (string*value) list option *)
            case (p, v) of
                (Wildcard          , _                ) => SOME(     [])
...
(* Problem 12. Write a function first_match that takes a value and a list of patterns and returns a
...
fun first_match v plist = (* -> (string * valu) list option *) 
    let 
        fun matchv(p) = match(v,p)
    in 
        SOME(first_answer matchv plist ) handle NoAnswer => NONE 
    end
