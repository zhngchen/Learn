...
;; Problem 2
(define (mupllist->racketlist v)
  (let ([v1 (apair-e1 v)]
        [v2 (apair-e2 v)])
    (cond [(aunit? v1) null]
          [(aunit? v2) (cons v1 null)]
  ...
  (cond [(var? e) 
        ...
        [(call? e)
         (let ([v1 (eval-under-env (call-funexp e) env)]
           ...
           (if (not (closure? v1))
               (error "MUPL first argument to a call must be a closure") ;; nameopt formal body
               (let ([function (closure-fun v1)])
                 (eval-under-env (fun-body function)
                                 (let ([itself (fun-nameopt function)]
                                       [param (cons (fun-formal function) v2)])
                                   (if itself
                                       (cons (cons itself v1) (cons param (closure-env v1)))
                                       (cons param (closure-env v1))))))))]
        [(apair? e)
...
(define mupl-map
  (fun #f "_f"
       (fun "_map" "_xs"
            (ifgreater (isaunit (var "_xs")) (int 0)
                       (aunit)
                       (apair
                        (call (var "_f") (fst (var "_xs")))
                        (call (var "_map") (snd (var "_xs"))))))))

(define mupl-mapAddN 
  (mlet "map" mupl-map
        (fun #f "_n"
             (call (var "map") (fun #f "x" (add (var "x") (var "_n")))))))
